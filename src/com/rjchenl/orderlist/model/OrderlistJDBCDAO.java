...
*/
	...
	public void insertWithOrderDetail(OrderlistVO orerlistvo, List<OrderdetailVO> list) {
		...
		try {
			...
			pstmt.setString(1, orerlistvo.getMem_id());
			pstmt.setString(2, orerlistvo.getStore_id());
			pstmt.setInt(3, orerlistvo.getOrd_total());
			pstmt.setInt(4, orerlistvo.getOrd_pick());
			pstmt.setString(5, orerlistvo.getOrd_add());
			...
			System.out.println("0711_step1");
			for (OrderdetailVO orderdetailVO : list) {
				System.out.println("0711_step2");
				orderdetailVO.setOrd_id(next_orderlist.toString());
				System.out.println("0711_step3");
				dao.insert2(orderdetailVO,con);
				System.out.println("0711_step4");
			}
			System.out.println("0711_step5");

			// 2●設定於 pstm.executeUpdate()之後
			con.commit();
			con.setAutoCommit(true);
			System.out.println("list.size()-B="+list.size());
			System.out.println("新增部門編號" + next_orderlist + "時,共有員工" + list.size()
					+ "人同時被新增");
